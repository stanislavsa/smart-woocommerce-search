...
function ysm_admin_enqueue_scripts() {
	...
	wp_enqueue_script( 'smart-search-admin', YSM_URI . 'assets/js/ysm-admin.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable', 'jquery-ui-slider', 'underscore', 'wp-color-picker', 'wp-util'), false, 1 );
...
/**
 * Init Default Settings on Plugin Activation
 */
function ysm_activation(){
	if (!get_option( 'smart_search_init', null )) {
		update_option( 'smart_search_init', 1 );
	}
}

register_activation_hook(__FILE__, 'ysm_activation');
