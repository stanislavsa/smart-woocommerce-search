...
class Ysm_Search {
     ...
     * @var array
     */
    protected static $protected_vars = array('post_types', 'taxonomies', 'postmeta');
    ...
    public static function init() {
        ...
        add_filter(
            'found_posts',
            array(__CLASS__, 'alter_found_posts'),
            PHP_INT_MAX,
            2
        );
        add_filter(
...
//        add_filter( 'woocommerce_get_filtered_term_product_counts_query', array(__CLASS__, 'alter_filtered_term_product_counts'), 10 );
    ...
    public static function search_filter( $query ) {
        ...
        if ( !is_admin() && !empty( $query->query_vars['s'] ) && !empty( $s ) && !(defined( 'DOING_AJAX' ) && DOING_AJAX) ) {
            ...
            if ( $w_id ) {
                ...
                self::parse_settings();
                if ( ysws_get_var( 'search_page_default_output' ) ) {
                    return $query;
                }
                if ( !self::get_post_types() ) {
                    return $query;
                }
                ysws_set_var( 'max_post_count', -1 );
                self::set_s( $s );
                if ( defined( 'POLYLANG_BASENAME' ) ) {
                    self::set_lang( pll_current_language() );
                } elseif ( defined( 'ICL_LANGUAGE_CODE' ) && ICL_LANGUAGE_CODE !== '' ) {
                    self::set_lang( ICL_LANGUAGE_CODE );
                }
                ...
                if ( ysws_get_var( 'search_page_suppress_filters' ) ) {
    ...
    public static function search_posts( $posts_count = 0, $offset = 0 ) {
        ...
        if ( false !== $cached ) {
            ...
            $posts = array();
            $postmeta = ysws_get_var( 'postmeta' );
            ...
            \YSWS\Core\Cache\set_query_cache( $key, $posts );
        }
        ...
        self::$processed[json_encode( self::get_search_terms() )] = $resulted_posts;
        /**
         * Modify the array of post ids
         *
         * @param array $post_ids List of post ids.
    ...
    public static function get_suggestions( $post_ids ) {
        ...
        foreach ( $post_ids as $post_id ) {
            ...
            if ( 'below_title' === ysws_get_var( 'popup_desc_pos' ) ) {
                $output .= $post_excerpt;
            }
            if ( $product ) {
            ...
            // holder closed
            $output .= '<div class="smart-search-clear"></div>';
            if ( 'below_image' === ysws_get_var( 'popup_desc_pos' ) ) {
                $output .= $post_excerpt;
            }
            $output .= '</div>';
    ...
    public static function set_lang( $lang ) {
        self::$vars['lang'] = wp_strip_all_tags( $lang );
    }
    ...
    public static function get_post_types( $type = '' ) {
        $post_types = (array) ysws_get_var( 'post_types' );
        if ( $type ) {
    ...
    protected static function set_search_terms() {
        ...
        if ( !ysws_get_var( 'enable_fuzzy_search' ) ) {
            $search_terms = [implode( ' ', $search_terms )];
        }
        foreach ( $search_terms as $search_term ) {
            if ( strlen( $search_term ) >= ysws_get_var( 'char_count' ) ) {
