...
function front_scripts() {
    wp_enqueue_style(
        'smart-search',
        SWS_PLUGIN_URI . 'assets/dist/css/general.css',
        array(),
        SWS_PLUGIN_VERSION
    );
    $l10n = [
    ...
    if ( defined( 'POLYLANG_BASENAME' ) ) {
        $l10n['restUrl'] = add_query_arg( array(
            'lang' => pll_current_language(),
        ), $l10n['restUrl'] ) . '&';
        $l10n['searchPageUrl'] = pll_home_url();
    } elseif ( defined( 'ICL_LANGUAGE_CODE' ) && ICL_LANGUAGE_CODE !== '' ) {
        $l10n['restUrl'] = add_query_arg( array(
            'lang' => ICL_LANGUAGE_CODE,
        ), $l10n['restUrl'] ) . '&';
        $l10n['searchPageUrl'] = apply_filters( 'wpml_home_url', $l10n['searchPageUrl'] );
    }
    $enhance_default_id = get_option( 'sws_enhance_default' );
    foreach ( ysm_get_all_widgets() as $k => $v ) {
        ...
        if ( $enhance_default_id && (string) $k === (string) $enhance_default_id ) {
            $css_classes['.widget_search'] = '.widget_search';
...
function admin_scripts() {
    ...
    wp_enqueue_style(
        'smart-search-admin',
        SWS_PLUGIN_URI . 'assets/dist/css/admin.css',
        [],
        SWS_PLUGIN_VERSION
    );
    ...
    wp_enqueue_script(
        'smart-search-admin',
        SWS_PLUGIN_URI . 'assets/dist/js/admin.js',
        array(
            'jquery',
            'jquery-ui-core',
            'jquery-ui-sortable',
    ...
    wp_localize_script( 'smart-search-admin', 'ysmProductSearch', [
        'ajax_url' => admin_url( 'admin-ajax.php' ),
    ] );
    // Select2
    wp_enqueue_style(
        'ysrs-select2',
        SWS_PLUGIN_URI . 'assets/dist/css/select2.min.css',
        array(),
        SWS_PLUGIN_VERSION
    );
    wp_enqueue_script(
        'ysrs-select2',
        SWS_PLUGIN_URI . 'assets/dist/js/select2.min.js',
        array(),
        SWS_PLUGIN_VERSION,
        true
    );
...
function admin_notification_styles() {
    wp_enqueue_style(
        'sws-notification',
        SWS_PLUGIN_URI . 'assets/dist/css/notification.css',
        array(),
        SWS_PLUGIN_VERSION
    );
}
